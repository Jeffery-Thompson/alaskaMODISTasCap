...
clusFile = 'AK_IntMon_Clu7.tif'
with rasterio.open(iDir3 +  clusFile, 'r', driver='GTiff', nodatavals = -9999.) as src:
    clIn = src.read()
    clMask = src.read_masks(1)
    clusters = clIn.squeeze()
...
pziClustFlat = clusters[pzi_in_tc_Col,pzi_in_tci_Row]
...
for i in range(len(ed)):
    ...
    tInd = np.where((pziFlat > st[i]) & (pziFlat < ed[i]) & (pziClustFlat == clSel) & (pziBrightFlat > -9999) & (pziGreenFlat > -9999) & (pziWetFlat > -9999) )
...
ax1.set_title('Avg. Tass. Cap Trends for Intermontane Boreal, Cluster ' + str(clSel))
